...
| 5 골든 픽스처 | ✅ | §7 — **20장** 세트(`golden_set.json`) 추가. 기존 1장도 유지 |
...
### 20장 세트 (2026-08-07 추가)

```
fixtures/
  golden_set.json    # 20장 기대 출력 (frames[] + _meta)
  images/            # 입력 20장
```

| | |
| --- | --- |
| 구성 | **REJECT 14 + PASS 6** — 결함만 모으면 "전부 결함으로 만드는" 회귀를 못 잡습니다 |
| 축 | x · y · z 라운드로빈 — 축마다 ROI 크기가 달라서(§3) 편중은 위험합니다 |
| 선정 | REJECT 는 **임계 여유가 큰** 순. 케이스마다 `_fx.margin` 을 같이 굳혀 둡니다 |
| 생성 · 검증 | `battery_infer_to_json.ipynb` **§7** 생성 / **§8** 회귀 |
| 기존 `golden_ct.*` | 그대로 둡니다 — 이미 쓰고 있는 회귀가 안 깨지게 |

### 통과 기준 — **게이트는 실패, 기하는 경고**

| 층 | 항목 | 기준 |
| --- | --- | --- |
| **실패** | `verdict` (PASS/REJECT) | **완전 일치** |
| **실패** | `num_detections` | **완전 일치** |
| **실패** | `detections[].class` | **완전 일치** |
| **실패** | `confidence` | **±0.01** |
| 경고 | `bbox_xyxy` 편차 · IoU | 기록만 (분포를 출력) |
| 경고 | `segmentation` | 면적 ±5% |

> ### ⚠️ bbox 를 실패 조건에서 뺀 이유 (2026-08-07)
>
> 제품이 파는 것은 **셀의 PASS/REJECT 판정**이고, **게이트는 bbox 를 쓰지 않습니다**(검출 유무만 씁니다).
>
> 게다가 CT 결함은 **폭 5px 슬리버**입니다(픽스처의 검출이 `5 x 292 px`). 스칼라 기하 임계가
> 이 모양에 안 맞습니다 — 같은 이동이 축에 따라 이렇게 다릅니다:
>
> | 이동 | IoU |
> | --- | --- |
> | x축 +1px | **0.667** |
> | x축 +6px | **0.000** (박스가 결함을 아예 안 덮음) |
> | y축 +6px | **0.960** |
>
> `±1px` 도 IoU 임계도 이 분포를 표현하지 못합니다. 그래서 §8 은 게이트 4항목만 실패로 두고,
> 기하는 **편차 분포**(완전 일치 몇 건 / dx·dy 최대 / IoU 최소)로 출력합니다.
> 그 분포가 ONNX 등 백엔드 전환의 허용치를 정하는 데이터입니다 — `CT_확인요청_회신.md` §2.
>
> **경고까지 실패로 만들면 픽스처가 매번 깨지고, 매번 깨지는 테스트는 아무도 안 봅니다.**

### 생성 · 검증

`battery_infer_to_json.ipynb` **§7** 가 20장 생성 / **§8** 이 회귀 검증입니다.
1장짜리 기존 픽스처는 **§5** 생성 / **§6** 검증으로 그대로 남아 있습니다.
픽스처는 **§2 가 실제로 낸 출력을 그대로 굳힙니다** — 픽스처용으로 따로 추론하면 배포 경로와 다른
코드를 검증하게 되므로 일부러 재추론하지 않습니다.
라이브러리 업그레이드·리팩터링 후에는 **§1 → §2 → §8** 순으로 돌리면 됩니다.
